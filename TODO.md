...
- [x] **Threads mesurés le 22/09** (extrait d'une minute, app en cours d'utilisation à côté) : défaut 78 s · 3 threads 94 s · 5 → 71 s · 8 → 78 s · 11 → 74 s · 16 → voir journal. Entre 5 et 16 threads tout tient dans le bruit (< 10 %) ; seul 3 threads perd nettement. **Réglage par défaut conservé.** `--threads N` reste dans la CLI.
