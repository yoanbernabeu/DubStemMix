...
**Renvois de bus à bus** (issue #1, validés le 23/09/2026) : le retour de chaque bus peut partir, en plus du master, dans **un autre bus choisi par l'utilisateur**, comme sur une console dub. Menu « Send to » sur la carte de chaque bus : None, Delay, Reverb ou Bus 3. Un potard par bus dose le renvoi, les trois en tranche 8 : celui du delay (ancien DLY→REV de la tranche 2, déplacé), puis ceux de la reverb et du bus 3. **Par défaut, chaque bus se suffit à lui-même** (validé le 25/09/2026) : aucune destination et potards à 0, chaîner des bus est un choix. Les projets enregistrés avant gardent leur DLY→REV (reverb, 20 %) et sonnent comme avant ; le routage est désormais toujours écrit dans le projet. Le libellé du potard affiche la destination (« REV→DLY », « PHS→— »). **Le bus 3 porte le nom de son effet** partout à l'écran (carte, en-tête de la page FX, page MIX, menus, réglages, potards) : PHASER / PHS avec le Bi-Phaser, FLANGER / FLG avec le Tape Flanger, FX 3 / FX3 avec un plugin. **Jamais de boucle** : un choix qui en fermerait une est grisé dans le menu, et un projet qui en contiendrait une revient au routage par défaut. Changer de destination arrête le moteur un instant (queues d'effets coupées), comme changer d'effet : c'est un réglage de préparation, les potards restent jouables en direct. Renvoi et retour d'un bus sont indépendants : retour à zéro, le renvoi passe quand même. Les potards de renvoi restent du routage quand un bus héberge un plugin : chaque bus a ses 6 macros, delay compris. Pas de renvoi depuis les inserts de tranche. **Rendre le câblage visible** (validé le 25/09/2026) : un renvoi se dessine en câble entre les cartes de bus, de la prise de sortie de la source à la prise d'entrée de la cible (pointillés tant que son potard est à zéro, signal qui circule quand la source sonne) ; chaque carte s'allume à la couleur de son bus tant que son effet sonne, queues comprises, et sa barre de couleur mesure ce qui entre dans le bus ; les potards de renvoi ont l'arc de leur source, l'aiguille et la flèche de leur cible, et sont atténués sans destination. Écarté pour l'instant : empiler plusieurs effets dans un même bus (voir l'issue).
...
- **Projet** = un morceau, fichier `.dubstem` (JSON) : références des stems, affectation aux tranches, fichiers en réserve, BPM, réglages et slots d'effets, boucle. **Enregistrer / Ouvrir classiques** : rien n'est écrit tant que l'utilisateur n'a pas fait « Save » et choisi l'emplacement (⌘S, ⇧⌘S pour « Save As ») ; ensuite les changements sont sauvegardés automatiquement dans ce fichier. Une session jamais enregistrée demande confirmation avant d'être remplacée. **« New » (et « Split a Song ») repart comme au lancement de l'app** (validé le 25/09/2026) : réglages d'effets aux valeurs d'usine, bus isolés, aucune tranche en mute ni en solo, en plus de la session vidée. Faders et envois restent là où les tient la console. Ouvrir un projet ne recâble pas les bus inutilement, pour que les queues d'effets continuent. Chaque fichier est référencé par chemin absolu **et** relatif au projet : le projet survit au déplacement du dossier entier.
...
- **Gestion de la setlist** (décidée le 25/09/2026) :
  - **Titre d'un morceau renommable** : double-clic sur le titre (barre du haut) ou « Rename… » au clic droit sur une ligne de la setlist. Le titre est enregistré dans le `.dubstem` et fait foi à la réouverture (il n'est plus recalculé depuis les noms de stems).
  - **Exporter la setlist** pour la jouer sur un autre Mac : un **dossier** (pas de zip, les WAV ne se compressent pas) qui contient la setlist, un sous-dossier par morceau avec son projet et **tous** ses fichiers (stems et fichiers en réserve), et un `PLUGINS.txt` s'il y a des plugins Audio Unit (ils ne se copient pas : l'autre Mac les installe, sinon ⚠ et effet intégré comme aujourd'hui). La copie se fait en tâche de fond, avec un compteur. Les stems introuvables ne sont pas copiés et sont signalés à la fin.
...
Carte son et taille de buffer · dossier d'enregistrement · mode pré/post par bus · gestion des plugins AU · vérification des mises à jour.

**Mises à jour** (23/09/2026, validées) : au lancement, au plus une fois par jour, l'app demande à GitHub la dernière release (rien d'autre n'est envoyé). Désactivable dans les Réglages (DAILY / OFF, CHECK NOW) ; Aide ▸ Check for Updates… à la main. Une version plus récente affiche un bandeau dans la barre du haut, **jamais pendant la lecture ni l'enregistrement**. **UPDATE** = un clic : l'app télécharge le zip, vérifie l'identifiant, la version et la signature, quitte (confirmation si la session n'a jamais été enregistrée), puis un petit script remplace l'app là où elle est installée (l'ancienne est remise en place si la copie échoue), retire la quarantaine comme `install.sh` et rouvre l'app. NOTES ouvre la page de la release ; la croix masque le bandeau jusqu'à la prochaine vérification. Une build de développement (`swift run`) ne vérifie qu'avec `DUBSTEMMIX_PRETEND_VERSION` et ouvre la page de la release au lieu de se remplacer. Choix écarté : Sparkle (dépendance et clés de signature à gérer pour une app sans compte Apple Developer).
