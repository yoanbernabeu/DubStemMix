...
public enum FXParameter: String, CaseIterable, Sendable {
    ...
    var kernelParameter: (effect: BuiltInEffect.Kind, index: Int)? {
        ...
        // Plate and spring share these (same indices), so do phaser and flanger: they go to the bus's switch.
        case .reverbDecay: (.reverbBus, DUB_PLATE_DECAY)
        case .reverbDamping: (.reverbBus, DUB_PLATE_DAMPING)
        case .reverbPredelay: (.reverbBus, DUB_PLATE_PREDELAY)
        case .reverbLowCut: (.reverbBus, DUB_PLATE_LOW_CUT)
        case .reverbTone: (.reverbBus, DUB_PLATE_TONE)
        case .phaserRate: (.bus3, DUB_PHASER_RATE)
        case .phaserDepth: (.bus3, DUB_PHASER_DEPTH)
        case .phaserFeedback: (.bus3, DUB_PHASER_FEEDBACK)
        case .phaserCenter: (.bus3, DUB_PHASER_CENTER)
        case .phaserStereo: (.bus3, DUB_PHASER_STEREO)
